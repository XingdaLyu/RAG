...
def build_elasticsearch(
    ...
    print(f'#files {len(pubmed_full_dataset["train"])}')
    config = {
    ...
    def generate_actions():
        ...
        for i in range(len(pubmed_full_dataset['train'])):
            doc = pubmed_full_dataset['train'][i]
            _id, text, title = doc['id'], doc['contents'], doc['title']
            es_doc = {
                '_id': _id,
                '_op_type': 'index',
                'refresh': 'wait_for',
...
if __name__ == '__main__':
    ...
    pubmed_full_dataset = load_dataset('MedRAG/pubmed')    

    build_elasticsearch(pubmed_full_dataset, index_name="pubmed")
